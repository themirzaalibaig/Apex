@php
    $mainImage = $heroSection?->mediaByType('main');
    $title1Image = $heroSection?->mediaByType('title1');
    $title2Image = $heroSection?->mediaByType('title2');
    $backgroundImage = $heroSection?->mediaByType('background');
    $heroImage = $heroSection?->mediaByType('hero');
@endphp

<div class="hero1-section-area"
    style="background-image: url({{ $backgroundImage?->url ?? asset('img/all-images/bg/hero-bg1.png') }}); background-position: center top; background-repeat: no-repeat; background-size: cover;">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-7">
                <div class="hero-header-area">
                    ...
                    <h1 class="text-anime-style-3">{{ $heroSection?->title1 ?? '' }}
                        @if ($title1Image)
                            <img src="{{ $title1Image->url }}" alt="" class="elements1 keyframe5" />
                        @endif
                        </h1>
                    ...
                    <h1 class="text-anime-style-3">
                        @if ($title2Image)
                            <img src="{{ $title2Image->url }}" alt="" class="others-img1" />
                        @endif
                        {{ $heroSection?->title2 ?? '' }}
                    </h1>
                </div>
            ...
            <div class="col-lg-4">
                <div class="imges-header">
                    <div class="img1">
                        <img src="{{ $mainImage?->url ?? asset('img/all-images/hero/hero-img1.png') }}" alt="" class="keyframe6" />
                    </div>
                    <div class="arrow">
            ...
            <div class="col-lg-12">
                <div class="hero-bottom-images">
                    ...
                    <div class="img1 image-anime reveal">
                        <img src="{{ $heroImage?->url ?? asset('img/all-images/hero/hero-img2.png') }}" alt="" />
                    </div>
                    <div class="started-btn">
                        ...
                        <a href="{{ $heroSection?->cta_url ?? '#' }}">{{ $heroSection?->cta ?? 'Get Started' }}</a>
                    </div>
                </div>
            </div>
        </div>
