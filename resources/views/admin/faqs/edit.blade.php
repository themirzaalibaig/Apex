        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-700/50">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Edit FAQ</h3>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">Update the FAQ information</p>
                    </div>
                </div>
            </div>

            <!-- Card Body -->
            <form action="{{ route('faqs.update', $faq->id) }}" method="POST" class="p-6 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <flux:textarea
                            ...
                            class="w-full"
                            rows="6"
                        >{{ old('answer', $faq->answer) }}</flux:textarea>
                        @error('answer')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <x-toggle-switch :status="$faq->status" name="status" />
                        @error('status')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-zinc-200 dark:border-zinc-700">
                    ...
                    <div class="flex items-center gap-3">
                        <flux:button
                            type="button"
                            variant="outline"
                            onclick="window.location.href='{{ route('faqs.index') }}'"
                            class="px-6"
                        >
