        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-700/50">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Edit Menu</h3>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400">Update the information for {{ $menu->name }}</p>
                    </div>
                </div>
            </div>

            <!-- Card Body -->
            <form action="{{ route('menus.update', $menu->id) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <x-toggle-switch :status="$menu->status" name="status" />
                        @error('status')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div id="submenusContainer" class="space-y-4">
                        @foreach($menu->subMenus as $index => $submenu)
                            <div class="submenu-item bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4" data-index="{{ $index }}" data-id="{{ $submenu->id }}">
                                <input type="hidden" name="submenus[{{ $index }}][id]" value="{{ $submenu->id }}">
                                ...
                                <div class="flex items-start justify-between mb-4">
                                    ...
                                    <button type="button" onclick="removeSubmenu({{ $index }}, {{ $submenu->id }})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="space-y-2">
                                        ...
                                        <input
                                            ...
                                            value="{{ old("submenus.{$index}.name", $submenu->name) }}"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input
                                            type="text"
                                            name="submenus[{{ $index }}][link]"
                                            value="{{ old("submenus.{$index}.link", $submenu->link) }}"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex items-center gap-3">
                                            <button type="button" onclick="openMenuMediaPicker({{ $index }})" class="inline-flex items-center px-4 py-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-zinc-900 dark:text-white rounded-lg border border-zinc-300 dark:border-zinc-600 text-sm font-medium transition-colors">
                                                <i class="fas fa-image mr-2"></i>
                                                Select Media
                                            </button>
                                            @php
                                                $submenuUsage = $submenu->mediaUsages->first();
                                                $submenuUpload = $submenuUsage?->upload;
                                            @endphp
                                            <input type="hidden" id="submenu_media_{{ $index }}" name="submenus[{{ $index }}][media_usages][0][upload_id]" value="{{ $submenuUsage?->upload_id }}">
                                        </div>
                                        <input
                                            type="text"
                                            id="submenu_media_type_{{ $index }}"
                                            name="submenus[{{ $index }}][media_usages][0][type]"
                                            value="{{ $submenuUsage?->type }}"
                                        ...
                                        />
                                        <div id="preview_{{ $index }}" class="{{ $submenuUpload ? '' : 'hidden' }} mt-2">
                                            <img id="preview_img_{{ $index }}" src="{{ $submenuUpload?->url }}" alt="Preview" class="w-24 h-24 object-cover rounded-lg border border-zinc-300 dark:border-zinc-600">
                                        </div>
                                    </div>
                    ...
                    <input type="hidden" name="submenusToDelete" id="submenusToDelete" value="[]">
                </div>
...
<script>
let submenuIndex = {{ $menu->subMenus->count() }};
let submenusToDelete = [];
...
function removeSubmenu(index, submenuId = null) {
    const submenu = document.querySelector(`.submenu-item[data-index="${index}"]`);
    if (submenu) {
        submenu.remove();

        // If it's an existing submenu, add to delete list
        if (submenuId) {
            submenusToDelete.push(submenuId);
            document.getElementById('submenusToDelete').value = JSON.stringify(submenusToDelete);
        }
    }
}
