                            ...
                            <div class="submenu-item bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg p-4" data-index="{{ $index }}" data-id="{{ $submenu->id }}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex items-center gap-3">
                                            <label for="submenu_image_{{ $index }}" class="cursor-pointer inline-flex items-center px-4 py-2 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-zinc-900 dark:text-white rounded-lg border border-zinc-300 dark:border-zinc-600 text-sm font-medium transition-colors">
                                                <i class="fas fa-image mr-2"></i>
                                                Choose Image
                                            </label>
                                            <input
                                                type="file"
                                                id="submenu_image_{{ $index }}"
                                                name="submenus[{{ $index }}][image]"
                                                accept="image/*"
                                                onchange="previewImage({{ $index }}, this)"
                                                class="hidden"
                                            />
                                        </div>
                                        <div id="preview_{{ $index }}" class="{{ $submenu->images ? '' : 'hidden' }} mt-2">
                                            <img id="preview_img_{{ $index }}" src="{{ $submenu->images ? Storage::url($submenu->images->image) : '' }}" alt="Preview" class="w-24 h-24 object-cover rounded-lg border border-zinc-300 dark:border-zinc-600">
                                        </div>
                                    </div>
