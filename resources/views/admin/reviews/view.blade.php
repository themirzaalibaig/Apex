...
<div class="py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <flux:heading size="xl">{{ $review->name }}</flux:heading>
                    <flux:text class="text-zinc-500 dark:text-zinc-400 mt-1">{{ $review->designation }}</flux:text>
                </div>
            </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-700/50">
                        <div class="flex items-center gap-2">
                            <flux:icon name="chat-bubble-left-right" class="w-5 h-5 text-blue-600 dark:text-blue-400" />
                            <h3 class="text-lg font-semibold text-zinc-900 dark:text-white">Review Details</h3>
                        </div>
                    </div>
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <p class="text-zinc-600 dark:text-zinc-400">{{ $review->email }}</p>
                        </div>

                        <!-- Rating -->
                        <div>
                            <flux:label class="text-zinc-700 dark:text-zinc-300 font-medium mb-2">Rating</flux:label>
                            <div class="flex items-center gap-2">
                                <div class="flex items-center">
                                    @for($i = 1; $i <= 5; $i++)
                                        <i class="fas fa-star {{ $i <= $review->rating ? 'text-yellow-400' : 'text-zinc-300 dark:text-zinc-600' }}"></i>
                                    @endfor
                                </div>
                                <span class="text-base font-semibold text-zinc-900 dark:text-white">{{ $review->rating }}/5</span>
                            </div>
                        </div>

                        <!-- Review Text -->
                        <div>
                            <flux:label class="text-zinc-700 dark:text-zinc-300 font-medium mb-2">Review Text</flux:label>
                            <p class="text-zinc-600 dark:text-zinc-400 whitespace-pre-line">{{ $review->review }}</p>
                        </div>
                    </div>
                </div>

                <!-- Media Gallery Card -->
                @if($review->mediaUsages->count() > 0)
                <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                    <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-700/50">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="ml-auto text-sm text-zinc-500 dark:text-zinc-400">{{ $review->mediaUsages->count() }} item(s)</span>
                        </div>
                    </div>

                    <div class="p-6">
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                            @foreach($review->mediaUsages as $usage)
                                @php
                                    $upload = $usage->upload;
                                @endphp
                                @if($upload)
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div class="pt-4 border-t border-zinc-200 dark:border-zinc-700">
                            ...
                            <p class="text-zinc-900 dark:text-white font-medium">
                                {{ $review->mediaUsages->count() }} item(s)
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-700 max-w-md w-full mx-4">
            <div class="p-6">
                ...
                <div class="flex items-center justify-end gap-3">
                    ...
                    <flux:button variant="danger" onclick="deleteReview()">
                        Delete Review
                    </flux:button>
                </div>
            </div>
    ...
    <script>
        ...
        function confirmDelete(reviewId, reviewName) {
        ...
        function deleteReview() {
            if (reviewToDelete) {
                ...
                form.method = 'POST';
                form.action = `/admin/reviews/${reviewToDelete}`;
