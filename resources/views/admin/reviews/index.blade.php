...
<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <flux:heading size="xl">Reviews Management</flux:heading>
            <flux:text class="mt-2">Manage customer reviews and testimonials</flux:text>
        </div>
        <flux:button variant="primary" icon="plus" href="{{ route('reviews.create') }}" wire:navigate>
            Add New Review
        </flux:button>
    </div>
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="flex items-center">
                                        @for($i = 1; $i <= 5; $i++)
                                            <i class="fas fa-star text-sm {{ $i <= $review->rating ? 'text-yellow-400' : 'text-zinc-300 dark:text-zinc-600' }}"></i>
                                        @endfor
                                    </div>
                                    <span class="ml-2 text-sm text-zinc-600 dark:text-zinc-400 font-medium">{{ $review->rating }}/5</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($review->mediaUsages->count() > 0)
                                    <div class="flex items-center gap-1">
                                        @foreach($review->mediaUsages->take(3) as $usage)
                                            @php
                                                $upload = $usage->upload;
                                            @endphp
                                            @if($upload)
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    <i class="fas fa-comment-dots text-4xl text-zinc-400 dark:text-zinc-500 mb-4"></i>
                                    ...
                                    <flux:button variant="primary" icon="plus" href="{{ route('reviews.create') }}">
                                        Add New Review
                                    </flux:button>
                                </div>
                            </td>
